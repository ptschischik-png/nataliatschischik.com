#!/usr/bin/env python3
"""
update-capi.py — Ersetzt den alten CAPI-Block in ALLEN HTML-Dateien.

Anleitung:
1. Lege diese Datei ins Root deines Repos
2. Führe aus: python3 update-capi.py
3. Prüfe die Änderungen mit: git diff
4. Commit + Push: git add -A && git commit -m "CAPI v2: fbp-Garantie + external_id" && git push
"""

import os
import glob

# ── Alter Block (exakt wie in deinen Dateien) ──
OLD_BLOCK = '''<!-- CAPI Server-Side Tracking -->
<script>
(function() {
...
# ── Neuer Block ──
NEW_BLOCK = '''<!-- CAPI Server-Side Tracking v2 — fbp-Garantie + external_id -->
<script>
(function() {
  var CAPI_WORKER_URL = "https://meta-capi.icy-sunset-af0f.workers.dev";
  ...
  // Das Meta Pixel SDK setzt _fbp, aber lädt async → oft noch nicht
  // da wenn PageView feuert. Wir generieren es vorher selbst.
  ...
  // Hilft Meta bei der Zuordnung auch ohne Login-Daten.
  // Bleibt 180 Tage im Cookie, wird SHA256-gehasht im Worker.
  ...
  // Sofort generieren (vor allem anderen)
  var fbp = ensureFbp();
  var externalId = ensureExternalId();

  // ── 3. sendCAPIEvent — angereichert mit allen Match-Keys ──
  window.sendCAPIEvent = function(eventName, eventId, customData, userData) {
    customData = customData || {};
    userData = userData || {};
...
def main():
    ...
    for filepath in sorted(html_files):
        ...
        # Schon aktualisiert?
        if "CAPI Server-Side Tracking v2" in content:
            already_done += 1
            print(f"  ⏭  {filepath} (bereits v2)")
            continue

        # Alten Block suchen und ersetzen
        if OLD_BLOCK in content:
            ...
            skipped += 1
            print(f"  ⚠️  {filepath} (CAPI-Block nicht gefunden)")

    print(f"\n{'='*50}")
    print(f"✅ Aktualisiert: {updated}")
    print(f"⏭  Bereits v2:   {already_done}")
    print(f"⚠️  Übersprungen:  {skipped}")
    print(f"{'='*50}")

    if updated > 0:
        print("\nNächste Schritte:")
        print("  1. git diff              → Änderungen prüfen")
        print("  2. git add -A")
        print('  3. git commit -m "CAPI v2: fbp-Garantie + external_id"')
        print("  4. git push")
